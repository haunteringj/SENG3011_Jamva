    ...
    axios.get("http://127.0.0.1:8000/v1/latestReports")
      .then((resp) => {
        ...
        for (const [country, cData] of Object.entries(countries)) {
          ...
          result.push({ lat: cData['lat'], lng: cData['long'], numReports: cData['reports'].length });
        }
        setc(cs);
        return result
      }).then((result) => {
