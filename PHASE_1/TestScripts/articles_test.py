...
def test_article_id_success():
    response = client.get("/articles/search/id?id=123")
    assert response.status_code == 200
...
def test_article_country_success():
    response = client.get("/articles/search/country?country=Chin a")
    assert response.status_code == 200
...
def test_article_country_no_such_article():
    response = client.get("/articles/search/country?country=moon")
    assert response.status_code == 404
    assert response.json() == '"no articles was found with that country. You entered:moon"'


def test_article_disease_success():
    response = client.get("/articles/search/disease?disease=Covid 19")
    assert response.status_code == 200
...
def test_article_disease_no_such_article():
    response = client.get("/articles/search/disease?disease=Cough")
    assert response.status_code == 404
